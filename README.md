...
- [Runtime Pipeline](docs/runtime-pipeline.md)：依赖关系、输出结构和恢复行为。
- [BabelDOC Runtime Profiles](docs/babeldoc-runtime-profiles.md)：托管翻译运行时、字体和 DirectML。
- [Complex PDF Live Matrix](docs/complex-pdf-live-matrix.md)：公式、OCR、翻译和压缩的发布/CI 验收。
...
- **Non-interactive mode**：已知参数或要批处理时使用
...
| 已有 Git checkout | `ocr-flow runtime setup --path <babeldoc-checkout>` | **破坏性操作**：清理并 detached 回退到 v0.6.3；再按所选 profile 安装。 |
...
| `4_gs_prepress_300dpi.pdf` | 1.7MB | 文字版，6页 | 公式/电路/图表/表格回归 | 严格 live matrix |
| `4_gs_prepress_300dpi_scanned_300dpi.pdf` | 7.3MB | 扫描版，6页 | UMI OCR + 公式视觉回归 | 严格 live matrix |
...
### 严格复杂 PDF Live Matrix（发布/CI 验收）

`4_gs_prepress_300dpi.pdf` 及其 300 DPI 扫描版用于真实端到端回归。它不会
使用模拟的 UMI OCR、BabelDOC、翻译 API、Ghostscript 或 MinerU；每个 profile 都会
实际消耗 24 个 MinerU 分段转换和 2 次翻译请求。缺少凭据或任何运行时依赖会失败，
不会跳过。
...
Windows 上应在 release/CI 验收中使用 `--all-profiles` 同时验证 CPU-safe 与
DirectML。输出位于 `output/live_complex_pdf_matrix/`，包含状态、PDF、Markdown
和公式/OCR/翻译页面渲染图。验收时必须查看渲染图，确认公式可见、中文字体正常、
版面无重叠、扫描件 OCR 有文字层。完整命令和验收标准见
[Complex PDF Live Matrix](docs/complex-pdf-live-matrix.md)。
...
```bash
ocr-flow runtime setup --path <babeldoc-checkout>
```
